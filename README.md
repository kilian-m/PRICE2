...
Every option is documented at its field in `price2/config.py`
...
Before anything is written, the output files are reconciled with that record: a half-written trailing line is dropped, and so is every row belonging to a locus that is re-run, so no result is duplicated or lost.

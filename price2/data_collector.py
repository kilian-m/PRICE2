...
class DataCollector:
    ...
    def collect_mappings(self,
        ...
        self.cur = self.transcript_read_counts_db.cursor()

        self.cur.execute('''CREATE TABLE IF NOT EXISTS transcript_read_counts (
                         locus_id text NOT NULL,
                         run_id text NOT NULL,
    ...
    def make_rgrs(
        ...
        for locus in self.loci_set:
            cur.execute('''SELECT * FROM transcript_read_counts
                        WHERE locus_id = ?''', (locus.id,))
            d = {}
            ...
            locus.keep_transcripts = [transcripts_dict[tr_id] for tr_id, count in explaining_transcripts_reads_list if count > min_explained_reads]
    ...
    for locus in loci_set:
        ...
        for alignment in br.fetch(locus.iv.chrom, locus.iv.start, locus.iv.end):
            ...
            for query_iv in rsa.genomic_region.intervals:
                for subject_iv, transcript_set in locus.transcript_intervals[query_iv].steps():
            ...
            if transcripts:
                ...
                if not mapping in mappings_dict:
        ...
        for entry, count in mappings_dict.items():
            ...
            for iv in ivs_tuple:
                ...
                is_first_iv = False
        df = pd.DataFrame(read_list, columns=['is_first_iv','start', 'end', 'untemplated_addition', 'unique', 'count'])
        ...
        cur_tr.execute('''INSERT INTO transcript_read_counts (
                         locus_id,
                         run_id,
                         transcript_read_counts_blob
                         ) VALUES (?, ?, ?)''', (locus.id, run.id, zlib.compress(dumps(transcripts_counts))))

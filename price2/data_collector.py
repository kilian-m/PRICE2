...
class DataCollector:
    ...
    def collect_mappings(self, reads_db_path:str):
        ...
        print(f'Collecting mappings for {len(self.loci_set)} loci and {len(self.runs)} runs')

        self.temp = []
        for run in self.runs:
            br = HTSeq.BAM_Reader(f'{self.bam_dir}/{run.id}.bam')
            print(f'Collecting mappings for run {run.id}')
            ...
            for locus in self.loci_set:
                ...
                self.cur.execute('''INSERT INTO reads (
                                 locus_id,
                                 run_id,
                                 reads_blob
                                 ) VALUES (?, ?, ?)''', (locus.id, run.id, dumps(df)))

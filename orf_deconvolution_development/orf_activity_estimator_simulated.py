...
class ORFActivityEstimator:
    ...
    def __init__(
        ...
        for step in self.regions_it.steps():
            ...
            if step_regions not in self.compatibility_groups:
                self.compatibility_groups[step_regions] = CompatibilityGroup()
            self.compatibility_groups[step_regions].length += step[0].length


    def fill_association_groups(self) -> None:

        # assign reads to association groups
        ...
        for read_al in self.bam_file:
            ...
            try:
                if not (region_frame, length, oua) in self.association_groups:
                    self.association_groups[(region_frame, length, oua)] = AssociationGroup()
                self.association_groups[(region_frame, length, oua)].read_count += 1 
                self.compatibility_groups[step_regions].read_count += 1
            except KeyError:
    ...
    def run_orf_deconvolution(self, iterations: int=0, likelihood_cutoff: int=200) -> None:
        ...
        for i in range(iterations):
            ...
            for (region_frame, length, oua),ac in self.association_groups.items():
                count = ac.read_count
                likelihoods = {}
    ...
    def incomplete_data_likelihood(self) -> float:
        ll = 0
        for cc_regions, cc in self.compatibility_groups.items():
    ...
    # product of likelihoods of all association groups (expected read count vs actual read count assuming poisson distribution)
    #def incomplete_data_likelihood_3(self) -> float:
    #    ll = 0
    #    for (region_frames, length, oua), ac in self.association_groups.items():
    #        l = 0
    #        try:
    #            ac_length = self.compatibility_groups[frozenset(x[0] for x in region_frames)].length
    #            expected_reads = 0
    ...
    def deactivate_region(self, rem_reg_id: str, inplace: bool=True):
        ...
        #        compatibility_groups[cc_regions] = cc
        #    else:
        #        cc_regions = frozenset([region_id for region_id in cc_regions if region_id != rem_reg_id])
        #        if not cc_regions in compatibility_groups:
        #            compatibility_groups[cc_regions] = Compatibility_group()
        #        compatibility_groups[cc_regions].read_count += cc.read_count
        #        compatibility_groups[cc_regions].length += cc.length
#
        ## association groups
        #association_groups = {}
        #for (region_frame, length, oua), ac in self.association_groups.items():
        #    if not rem_reg_id in [x[0] for x in region_frame]:
        #        if not (region_frame, length, oua) in association_groups:
        #            association_groups[(region_frame, length, oua)] = ac
        #        else:
        #            association_groups[(region_frame, length, oua)].read_count += ac.read_count
        #    else:
        #        region_frame = frozenset([x for x in region_frame if x[0] != rem_reg_id])
        #        if not (region_frame, length, oua) in association_groups:
        #            association_groups[(region_frame, length, oua)] = Association_group()
        #        association_groups[(region_frame, length, oua)].read_count += ac.read_count
        ...
        if inplace:
            ...
            #self.association_groups = association_groups
